...
async function renderFCSScoreboard(){
  ...
  const rankLookup=top25.reduce((map,t)=>{const rank=String(t?.rank||'').trim();const name=cleanTeamLabel(t?.team||t?.name||'');if(rank&&name)map.set(norm(name),rank);return map;},new Map());
  function rankForTeam(t,fallback=''){
    const name=teamName(t,fallback);
    const direct=rankLookup.get(norm(name));
    if(direct)return direct;
    const hit=top25.find(item=>teamMatch(name,item?.team||item?.name||''));
    return hit?.rank!=null?String(hit.rank):'';
  }
  ...
  const FCS_LOGO_IDS={
    ...
    Nevada:'2440',Colorado:'38','South Dakota':'233','Wyoming':'2751','Colorado State':'36',Utah:'254',Oregon:'2483','Oregon State':'204','Washington State':'265',Washington:'264','San Jose State':'23','San José State':'23','Utah State':'328','Boise State':'68','Fresno State':'278','San Diego State':'21','South Dakota State':'2569','North Dakota State':'2449','Montana State (57)':'147','North Dakota':'155','Lamar':'2320','Incarnate Word':'2916','SMU':'256', 'San Diego':'301','Yale':'43','West Florida':'290','Harvard':'108','Tarleton State':'2627','Illinois State':'2287','Lehigh':'2329','Rhode Island':'227','Youngstown State':'2754','Stephen F. Austin':'2617','Tennessee Tech':'2638','Austin Peay':'2041','Central Connecticut State':'211','Stetson':'56'
  };
  const bigSkyAliases={
  ...
  function normalizeEvent(ev){
    ...
    const status=c.status||{};const st=status.type||{};const broadcasts=[];(c.broadcasts||[]).forEach(b=>(b.names||[]).forEach(n=>broadcasts.push(n)));
    const odds=Array.isArray(c.odds)?c.odds[0]:(c.odds||null);
    return {id:String(ev.id||''),date:ev.date||c.date||'',teams,state:st.state||status.state||'',completed:!!(st.completed||status.completed),period:Number(st.period||status.period||0),clock:String(st.displayClock||status.displayClock||'').trim(),detail:st.shortDetail||st.detail||status.detail||'',broadcasts:broadcasts.slice(0,3),odds,raw:ev};
  }
  ...
  function scoreCardTeam(t,fallback,showScore,rank=''){
    ...
    const rankHtml=rank?`<span class="fcs-team-rank">#${escapeHtml(rank)}</span>`:'';
    return `<div class="score-team-row ghq-fcs-team-row">${rankHtml}${logo?`<img src="${escapeHtml(logo)}" alt="" loading="lazy" onerror="this.style.display='none'">`:''}<span>${escapeHtml(name)}</span>${showScore?`<strong>${escapeHtml(t?.score??'—')}</strong>`:''}</div>`;
  }
  ...
  function gameCard(game,ev){
    ...
    return `<div class="fcs-game ${state} bigsky-row ghq-fcs-game" data-fcs-game-key="${escapeHtml(game.matchupKey)}"><div class="fcs-time"><b>${escapeHtml(game.date)}</b><small>${escapeHtml(label)}</small></div><div class="fcs-matchup">${scoreCardTeam(a,awayName,final||live,rankForTeam(a,awayName))}${scoreCardTeam(h,homeName,final||live,rankForTeam(h,homeName))}<small class="score-game-status">${escapeHtml(status)}${tv?' • '+escapeHtml(tv):''}</small></div><div class="fcs-score score-status">${final||live?`<span class="score-big">${escapeHtml(a?.score??'—')}–${escapeHtml(h?.score??'—')}</span><small>${escapeHtml(final?'FINAL':status)}</small>`:`<small>${escapeHtml(status)}</small>`}</div><div class="fcs-tv">${escapeHtml(tv)}</div><div class="bigsky-info bigsky-betting"><b>CHECKING…</b><small>MARKET LINES</small></div></div>`;
  }
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id='ghq-fcs-scoreboard-surgical-styles';s.textContent=`
      .ghq-fcs-rank-card{overflow:hidden!important}.ghq-fcs-top-matchup{min-width:0!important}.ghq-fcs-rank-heading{display:flex!important;align-items:center!important;gap:10px!important;width:100%!important;min-height:30px!important}.ghq-fcs-rank-heading b{display:inline-block!important;visibility:visible!important;opacity:1!important;color:#151515!important;font-size:17px!important;font-weight:800!important;line-height:1.2!important;white-space:normal!important}.ghq-fcs-rank-heading strong{margin-left:auto!important;color:#151515!important}.ghq-fcs-scheduled{display:flex!important;gap:8px!important;align-items:center!important;margin:9px 0 4px!important;font-size:14px!important;color:#151515!important}.ghq-fcs-scheduled span{opacity:.55!important}.ghq-fcs-team-row{display:flex!important;align-items:center!important;min-width:0!important;gap:8px!important}.ghq-fcs-team-row span{display:block!important;visibility:visible!important;opacity:1!important;color:#151515!important;font-weight:700!important;white-space:normal!important}.ghq-fcs-team-row img{width:26px!important;height:26px!important;object-fit:contain!important;flex:0 0 26px!important}.ghq-fcs-team-row .fcs-team-rank{display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:0 0 auto!important;min-width:27px!important;height:22px!important;padding:0 5px!important;border-radius:5px!important;background:#8c1531!important;color:#fff!important;font-size:12px!important;font-weight:900!important;line-height:1!important;white-space:nowrap!important}.ghq-fcs-team-row strong{margin-left:auto!important}.ghq-fcs-scheduled-teams{display:flex!important;align-items:center!important;gap:8px!important}.ghq-fcs-scheduled-teams .ghq-fcs-team-row{min-width:0!important;flex:1 1 0!important}.ghq-fcs-scheduled-teams .ghq-fcs-team-row span{font-size:13px!important}.ghq-fcs-game .bigsky-betting{min-width:180px!important}.ghq-fcs-game .fcs-matchup{min-width:0!important}.ghq-fcs-game .score-game-status{display:block!important;visibility:visible!important;opacity:1!important;font-size:11px!important;font-weight:800!important;color:#8c1531!important;margin-top:6px!important;line-height:1.2!important;white-space:normal!important}.ghq-fcs-game .fcs-matchup .fcs-team-rank{display:inline-flex!important;visibility:visible!important;opacity:1!important;align-items:center!important;justify-content:center!important;min-width:24px!important;height:19px!important;padding:0 4px!important;margin-right:5px!important;border-radius:4px!important;background:#8c1531!important;color:#fff!important;font-size:11px!important;font-weight:900!important;line-height:1!important}.ghq-fcs-game.live .score-game-status{font-size:12px!important}.ghq-fcs-game.live .score-game-status:before{content:'LIVE • '!important}
  ...
  draw();
  if(!grizFcsRefreshTimer){
    grizFcsRefreshTimer=setInterval(()=>{if(new Date().getDay()>=4)draw();},60000);
  }
}
