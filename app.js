...
async function renderBigSkyAndOpponent(){
  try{
    ...
    function weekForDate(label){
      ...
      const dt=new Date(`2026-${String({Aug:8,Sep:9,Oct:10,Nov:11}[m]).padStart(2,"0")}-${day.padStart(2,"0")}T12:00:00`);
      const daysToSat=6-dt.getDay(); dt.setDate(dt.getDate()+daysToSat);
    ...
    if(t&&games.length){
      ...
      function draw(){
        ...
        if(selectedWeek!=="ALL") a=a.filter(x=>weekForDate(x.date)===selectedWeek);
        if(ts?.value && ts.value!=="ALL") a=a.filter(x=>x.team===ts.value||x.opponent===ts.value);
        a.sort((x,y)=>x.date.localeCompare(y.date));
        if(!a.length){t.innerHTML='<div class="bigsky-empty"><strong>No Big Sky team games this week.</strong><span>Use the WEEK dropdown to look ahead.</span></div>';return;}
        t.innerHTML='<div class="bigsky-row bigsky-head"><span>DATE</span><span>BIG SKY TEAM</span><span>OPPONENT</span><span>TIME</span></div>'+a.map(x=>{
          const bs=x.team;
          const away=x.location==='Away';
          const conference=x.big_sky_game;
          return '<div class="bigsky-row"><span>'+escapeHtml(x.date)+'</span><b>'+escapeHtml(bs)+'</b><span>'+(away?'@ ':'vs ')+escapeHtml(x.opponent)+(conference?' <small class="league-tag">BIG SKY</small>':'')+'</span><span>'+escapeHtml(x.time)+'</span></div>';
        }).join('');
      }
