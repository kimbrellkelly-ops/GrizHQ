...
async function renderBigSkyAndOpponent(){try{const d=await (await fetch("data.json?ts="+Date.now(),{cache:"no-store"})).json();const t=document.getElementById("bigsky-table"),ts=document.getElementById("bigsky-team-filter"),ds=document.getElementById("bigsky-date-filter");if(t&&d.big_sky_schedule){(d.big_sky_teams||[]).forEach(x=>{let o=document.createElement("option");o.value=x;o.textContent=x.toUpperCase();ts.appendChild(o)});[...new Set(d.big_sky_schedule.map(x=>x.date))].forEach(x=>{let o=document.createElement("option");o.value=x;o.textContent=x;ds.appendChild(o)});function draw(){let a=d.big_sky_schedule;if(ts.value!=="ALL")a=a.filter(x=>x.away===ts.value||x.home===ts.value);if(ds.value!=="ALL")a=a.filter(x=>x.date===ds.value);t.innerHTML='<div class="bigsky-row bigsky-head"><span>DATE</span><span>MATCHUP</span><span>TIME</span><span>TV</span></div>'+a.map(x=>'<div class="bigsky-row"><span>'+escapeHtml(x.date)+'</span><b>'+escapeHtml(x.away)+' @ '+escapeHtml(x.home)+'</b><span>'+escapeHtml(x.time)+'</span><span>'+escapeHtml(x.tv)+'</span></div>').join("")}ts.onchange=draw;ds.onchange=draw;draw()}const n=d.next_game?.opponent||"Drake",r=d.opponent_resources?.[n];if(r){document.getElementById("opponent-hub-name").textContent=n;document.getElementById("opp-title").textContent=n.toUpperCase();document.getElementById("opp-official").href=r.official;document.getElementById("opp-forum").href=r.forum;document.getElementById("opp-forum-label").textContent=r.label||"Fan discussion";document.getElementById("opp-media").href=r.media}}catch(e){console.warn(e)}}renderBigSkyAndOpponent();
