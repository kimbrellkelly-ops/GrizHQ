...
async function renderFCSScoreboard(){
  ...
  const today=new Date();
  if(!weekEl.dataset.userChanged){let idx=weeks.findIndex(w=>today>=new Date(w[0]+'T00:00:00')&&today<=new Date(w[1]+'T23:59:59'));if(idx<0)idx=weeks.findIndex(w=>new Date(w[0])>today);weekEl.value=String(idx<0?weeks.length-1:idx)}
  const w=weeks[Number(weekEl.value)||0];
  let data={};
  try{data=await (await fetch('data.json?ts='+Date.now(),{cache:'no-store'})).json()}catch(e){if(statusEl)statusEl.textContent='Score data unavailable';return}
  const rankings=Array.isArray(data.fcs_top25)?data.fcs_top25:(Array.isArray(data.fcs_top20)?data.fcs_top20:[]);
  const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const clean=s=>String(s||'').replace(/\s*\([^)]*\)\s*$/,'').replace(/\s+/g,' ').trim();
  const norm=s=>clean(s).toLowerCase().replace(/[^a-z0-9]/g,'');
  const aliases={'southdakotastate':['southdakotastate','s dakota st','s dakota state','south dakota st'],'youngstownstate':['youngstownstate','youngstownst'],'rhodeisland':['rhodeisland','rhodeislandrams'],'stephenfaustin':['stephenfaustin','stephenf austin','sfa'],'austinpeay':['austinpeay','austinpeaygovernors'],'williammary':['williammary','williamandmary'],'westflorida':['westflorida','westfloridaargos'],'southernillinois':['southernillinois','southernillinois salukis']};
  const same=(a,b)=>{const x=norm(a),y=norm(b);if(!x||!y)return false;if(x===y)return true;for(const vals of Object.values(aliases)){if(vals.includes(x)&&vals.includes(y))return true}return (x.length>5&&y.length>5&&(x.startsWith(y)||y.startsWith(x)))&&!(['montana','montanastate','idaho','idahostate','southdakota','southdakotastate','northdakota','northdakotastate'].includes(x)&&['montana','montanastate','idaho','idahostate','southdakota','southdakotastate','northdakota','northdakotastate'].includes(y));};
  const idMap={'Montana':'149','Montana State':'147','South Dakota State':'2569','South Dakota':'233','North Dakota':'155','North Dakota State':'2449','Lehigh':'439','Rhode Island':'227','Youngstown State':'2754','Tarleton State':'2680','Illinois State':'2287','UC Davis':'302','Stephen F. Austin':'2617','Tennessee Tech':'2638','Austin Peay':'2046','Lamar':'2320','Idaho State':'304','West Florida':'290','South Carolina State':'256','Yale':'43','Utah Tech':'3101','Northern Arizona':'2464','William & Mary':'2729','Villanova':'222','Mercer':'2382','Southern Illinois':'79'};
  const logo=t=>t?.logo||t?.team?.logo||(t?.id?`https://a.espncdn.com/i/teamlogos/ncaa/500/${encodeURIComponent(t.id)}.png`:idMap[t?.name]?`https://a.espncdn.com/i/teamlogos/ncaa/500/${idMap[t.name]}.png`:'');
  const teamName=t=>t?.shortDisplayName||t?.displayName||t?.name||t?.abbreviation||'Team';
  const fetchDays=[];for(let d=new Date(w[0]+'T12:00:00');d<=new Date(w[1]+'T12:00:00');d.setDate(d.getDate()+1))fetchDays.push(`${d.getFullYear()}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}`);
  const events=[];const seen=new Set();
  for(const day of fetchDays){for(const host of ['https://site.api.espn.com','https://site.web.api.espn.com']){try{const r=await fetch(`${host}/apis/site/v2/sports/football/college-football/scoreboard?dates=${day}&limit=1000`,{cache:'no-store'});if(!r.ok)continue;const p=await r.json();const list=Array.isArray(p.events)?p.events:[];for(const ev of list){if(!seen.has(String(ev.id))){seen.add(String(ev.id));events.push(ev)}}if(list.length)break}catch(e){}}}
  const ranked=rankings.slice(0,25);
  const eventTeams=ev=>(ev?.competitions?.[0]?.competitors||[]).map(c=>({id:String(c.id||c.team?.id||''),name:c.team?.displayName||'',shortDisplayName:c.team?.shortDisplayName||'',abbreviation:c.team?.abbreviation||'',homeAway:c.homeAway||'',score:c.score??'',logo:c.team?.logo||''}));
  const eventFor=name=>events.find(ev=>eventTeams(ev).some(t=>same(name,teamName(t))||String(t.id)===String(idMap[name]||'')));
  const status=ev=>{const s=ev?.competitions?.[0]?.status?.type||{};if(s.completed)return 'FINAL';if(s.state==='in'||s.name==='STATUS_IN_PROGRESS')return s.shortDetail||'LIVE';return s.shortDetail||s.detail||'SCHEDULED'};
  const card=t=>{const name=clean(t.team||t.name),rank=t.rank||'',ev=eventFor(name);if(!ev)return `<div class="fcs-rank-card ghq-fcs-rank-card"><div class="ghq-fcs-top-scorecard"><div class="ghq-fcs-top-team-row"><span class="fcs-team-rank">#${esc(rank)}</span><span class="ghq-fcs-top-logo-slot"></span><span>${esc(name)}</span></div><div class="ghq-fcs-top-meta"><span>SCHEDULE DATA UNAVAILABLE</span></div></div></div>`;const ts=eventTeams(ev),a=ts.find(x=>x.homeAway==='away')||ts[0],h=ts.find(x=>x.homeAway==='home')||ts[1];const row=x=>`<div class="ghq-fcs-top-team-row"><span class="fcs-team-rank">${String(x.id)===String(idMap[name])?'#'+esc(rank):''}</span><span class="ghq-fcs-top-logo-slot">${logo(x)?`<img src="${esc(logo(x))}" alt="" loading="lazy">`:''}</span><span>${esc(teamName(x))}</span><strong>${esc(x.score||'—')}</strong></div>`;return `<div class="fcs-rank-card ghq-fcs-rank-card"><div class="ghq-fcs-top-scorecard">${row(a)}${row(h)}<div class="ghq-fcs-top-meta"><span>${esc(status(ev))}</span></div></div></div>`};
  topEl.innerHTML=ranked.map(card).join('')||'<div class="fcs-loading">Rankings unavailable.</div>';
  const bsTeams=new Set(['Montana','Montana State','Idaho','Weber State','Eastern Washington','Northern Arizona','Northern Colorado','Idaho State','Cal Poly','Southern Utah','Utah Tech','UC Davis','Portland State']);
  const sched=Array.isArray(data.big_sky_full_schedules)?data.big_sky_full_schedules:[];
  const bs=sched.filter(g=>{const dt=String(g.date||'');return dt>=w[0]&&dt<=w[1]&&bsTeams.has(g.team)});
  if(bigSkyEl)bigSkyEl.innerHTML=bs.map(g=>`<div class="fcs-game ghq-fcs-game"><div class="fcs-time"><b>${esc(g.date||'')}</b><small>${esc(g.time||'TBA')}</small></div><div class="fcs-matchup"><div>${esc(g.location==='Away'?g.opponent:g.team)}</div><div>${esc(g.location==='Away'?g.team:g.opponent)}</div><small>${esc(g.network||'')}</small></div><div class="fcs-score"><small>${esc(g.result||g.status||g.time||'SCHEDULED')}</small></div></div>`).join('')||'<div class="fcs-loading">No Big Sky games listed for this week.</div>';
  if(statusEl)statusEl.textContent=`Loaded ${events.length} ESPN events for ${w[2]}`;
  weekEl.onchange=()=>{weekEl.dataset.userChanged='1';renderFCSScoreboard()};
}
