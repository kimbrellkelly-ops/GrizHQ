...
async function renderFCSScoreboard(){
  ...
  const today=new Date();
  if(!weekEl.dataset.userChanged){
    let idx=weeks.findIndex(w=>today>=new Date(w[0]+'T00:00:00')&&today<=new Date(w[1]+'T23:59:59'));
    if(idx<0)idx=weeks.findIndex(w=>new Date(w[0])>today);
    weekEl.value=String(idx<0?weeks.length-1:idx);
  }
  ...
  const aliases={
    southdakotastate:['southdakotastate','southdakotast','sdstate','s dakota state'],
    montanastate:['montanastate','montanast','montana state'],
    northdakotastate:['northdakotastate','northdakotast'],
    youngstownstate:['youngstownstate','youngstownst'],
    stephenfaustin:['stephenfaustin','stephenf austin','sfa'],
    williammary:['williammary','williamandmary'],
    westflorida:['westflorida','westfloridaargos'],
    southernillinois:['southernillinois','southernillinoissalukis'],
  ...
  const same=(a,b)=>{
    ...
    if(!x||!y)return false;
    if(x===y)return true;
    for(const vals of Object.values(aliases))if(vals.includes(x)&&vals.includes(y))return true;
    return false;
  };
  const idMap={
    'Montana':'149','Montana State':'147','South Dakota State':'2569','South Dakota':'233',
    'North Dakota':'155','North Dakota State':'2449','Lehigh':'439','Rhode Island':'227',
    'Youngstown State':'2754','Tarleton State':'2680','Illinois State':'2287','UC Davis':'302',
    'Stephen F. Austin':'2617','Tennessee Tech':'2638','Austin Peay':'2046','Lamar':'2320',
    'Idaho State':'304','West Florida':'290','South Carolina State':'256','Yale':'43',
    'Utah Tech':'3101','Northern Arizona':'2464','William & Mary':'2729','Villanova':'222',
    'Mercer':'2382','Southern Illinois':'79','Eastern Washington':'331'
  };
  const fetchDays=[];
  for(let d=new Date(w[0]+'T12:00:00');d<=new Date(w[1]+'T12:00:00');d.setDate(d.getDate()+1))
    fetchDays.push(`${d.getFullYear()}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}`);

  let dataCache={};
  try{dataCache=await (await fetch('data.json?ts='+Date.now(),{cache:'no-store'})).json()}
  catch(e){if(statusEl)statusEl.textContent='Rankings data unavailable';return}
  const ranked=Array.isArray(dataCache.fcs_top25)?dataCache.fcs_top25:[];
  const rankings=ranked.length?ranked:(Array.isArray(dataCache.fcs_top20)?dataCache.fcs_top20:[]);

  const events=[],seen=new Set();
  for(const day of fetchDays){
    let got=false;
    for(const url of [
      `https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?dates=${day}&groups=20&limit=1000`,
      `https://site.web.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?dates=${day}&groups=20&limit=1000`,
      `https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?dates=${day}&limit=1000`
    ]){
      try{
        const r=await fetch(url,{cache:'no-store'});
        if(!r.ok)continue;
        ...
        got=true;
        if(Array.isArray(p.events)&&p.events.length)break;
      }catch(e){}
    }
    if(!got)console.warn('ESPN scoreboard unavailable for',day);
  }

  const eventTeams=ev=>(ev?.competitions?.[0]?.competitors||[]).map(c=>({
  ...
  const eventFor=name=>events.find(ev=>eventTeams(ev).some(t=>String(t.id)===String(idMap[name]||'')||same(name,teamName(t))||same(name,t.name)));
  ...
  const card=t=>{
    const name=clean(t.team||t.name),rank=t.rank||'',ev=eventFor(name);
    if(!ev)return `<div class="fcs-rank-card ghq-fcs-rank-card"><div class="ghq-fcs-top-scorecard"><div class="ghq-fcs-top-team-row"><span class="fcs-team-rank">#${esc(rank)}</span><span class="ghq-fcs-top-logo-slot"></span><span>${esc(name)}</span></div><div class="ghq-fcs-top-meta"><span>SCHEDULE DATA UNAVAILABLE</span></div></div></div>`;
    const ts=eventTeams(ev),a=ts.find(x=>x.homeAway==='away')||ts[0],h=ts.find(x=>x.homeAway==='home')||ts[1]||ts[0];
    const row=x=>`<div class="ghq-fcs-top-team-row"><span class="fcs-team-rank">${String(x.id)===String(idMap[name])?'#'+esc(rank):''}</span><span class="ghq-fcs-top-logo-slot">${logo(x)?`<img src="${esc(logo(x))}" alt="${esc(teamName(x))}" loading="lazy" onerror="this.style.display='none'">`:''}</span><span>${esc(teamName(x))}</span><strong>${esc(x.score||'—')}</strong></div>`;
    return `<div class="fcs-rank-card ghq-fcs-rank-card"><div class="ghq-fcs-top-scorecard">${row(a)}${row(h)}<div class="ghq-fcs-top-meta"><span>${esc(eventStatus(ev))}</span></div></div></div>`;
  };
  topEl.innerHTML=rankings.slice(0,25).map(card).join('')||'<div class="fcs-loading">Rankings unavailable.</div>';
  ...
  const bsEvents=events.filter(ev=>eventTeams(ev).some(t=>[...bsTeams].some(n=>same(n,teamName(t)))));
  const rendered=new Set();
  const bsCards=[];
  for(const ev of bsEvents){
    if(rendered.has(String(ev.id)))continue;
    rendered.add(String(ev.id));
    const ts=eventTeams(ev),a=ts.find(x=>x.homeAway==='away')||ts[0],h=ts.find(x=>x.homeAway==='home')||ts[1]||ts[0];
    const row=x=>`<div>${logo(x)?`<img class="ghq-fcs-inline-logo" src="${esc(logo(x))}" alt="${esc(teamName(x))}" loading="lazy" onerror="this.style.display='none'">`:''}${esc(teamName(x))}</div>`;
    bsCards.push(`<div class="fcs-game ghq-fcs-game"><div class="fcs-time"><b>${esc(ev.date?new Date(ev.date).toLocaleDateString(undefined,{month:'short',day:'numeric'}):'')}</b><small>${esc(eventStatus(ev))}</small></div><div class="fcs-matchup">${row(a)}${row(h)}<small>${esc(ev.name||'')}</small></div><div class="fcs-score"><small>${esc((a.score||h.score)?`${a.score||'—'}–${h.score||'—'}`:eventStatus(ev))}</small></div></div>`);
  }
  if(bigSkyEl)bigSkyEl.innerHTML=bsCards.join('')||'<div class="fcs-loading">No Big Sky games found for this week.</div>';
  if(statusEl)statusEl.textContent=`Loaded ${events.length} ESPN FCS events for ${w[2]}`;
