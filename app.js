...
async function renderFCSScoreboard(){
  ...
  function eventsForWeek(w){
    ...
    const seen=new Set();
    return out.filter(ev=>{const id=String(ev.id||'')||JSON.stringify(ev);if(seen.has(id))return false;seen.add(id);return true;});
  }

  function scheduleDate(label){
    const m={Aug:7,Sep:8,Oct:9,Nov:10};
    const parts=String(label||'').trim().split(/\s+/);
    return parts.length===2&&m[parts[0]]!=null?new Date(2026,m[parts[0]],Number(parts[1]),12):null;
  }

  function scheduledBigSkyGames(w){
    ...
    fullSchedule.forEach(g=>{
      ...
      if(!bigSkyTeams.includes(g.team))return;
      const away=g.location==='Away'?g.opponent:g.team;
      const home=g.location==='Away'?g.team:g.opponent;
      if(!away||!home)return;
      const key=`${g.date}|${[norm(away),norm(home)].sort().join('|')}`;
      if(byGame.has(key))return;
      byGame.set(key,{...g,displayAway:away,displayHome:home,matchupKey:key});
    });
    return [...byGame.values()];
  }

  function scheduledForTeam(name,games){
    const n=norm(name);
    return games.find(g=>norm(g.displayAway)===n||norm(g.displayHome)===n)||null;
  }

  function eventMatchesScheduled(ev,game){
    const ts=ev?.teams||[];
    if(ts.length<2||!game)return false;
    const away=ts.find(t=>t.homeAway==='away')||ts[0];
    const home=ts.find(t=>t.homeAway==='home')||ts[1];
    return teamMatches(game.displayAway,away?.name||away?.short||'')&&teamMatches(game.displayHome,home?.name||home?.short||'');
  }

  async function fetchLiveEvents(w){
    const start=w[0].replace(/-/g,'');
    const end=w[1].replace(/-/g,'');
    const urls=[
      `https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?dates=${start}-${end}&groups=81&limit=500`,
      `https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?dates=${start}-${end}&limit=500`
    ];
  ...
  function mergeEvents(cached,live){
    const map=new Map();
    [...cached,...live].forEach(ev=>{const id=String(ev.id||'')||JSON.stringify(ev);map.set(id,ev);});
    return [...map.values()];
  }
  ...
  async function draw(){
    ...
    const cached=eventsForWeek(w);
    const live=await fetchLiveEvents(w);
    const events=mergeEvents(cached,live);
    const scheduled=scheduledBigSkyGames(w);
    statusEl.textContent=`${events.length} FCS games checked • schedule + live data`;

    topEl.innerHTML=top25.slice(0,25).map(t=>{
      const scheduledGame=scheduledForTeam(t.team,scheduled);
      const ev=findTeamEvent(t.team,events) || (scheduledGame ? events.find(e=>eventMatchesScheduled(e,scheduledGame)) : null);
      const isGriz=norm(t.team)==='montana';
      const isBigSky=bigSkyTeams.includes(t.team);
      let matchup='No game this week';
      let detail=null;
      if(ev){
        detail=gameLabel(ev);
        matchup=`${escapeHtml(detail.away)} @ ${escapeHtml(detail.home)}`;
      }else if(scheduledGame){
        matchup=`${escapeHtml(scheduledGame.displayAway)} @ ${escapeHtml(scheduledGame.displayHome)}`;
      }
      const cardClass=isGriz?'griz':(isBigSky?'bigsky':'');
      const score=ev?scoreLine(ev,t.team):`<small>${escapeHtml(scheduledGame?.time||'UPCOMING')}</small>`;
      return `<div class="fcs-rank-card ${cardClass}"><span class="fcs-rank">${escapeHtml(t.rank)}</span><div class="fcs-rank-team"><b>${escapeHtml(t.team)}</b><small>${escapeHtml(t.record||'')} • ${matchup}</small></div><span class="fcs-rank-score">${score}</span></div>`;
    }).join('');

    if(bigSkyEl){
      bigSkyEl.innerHTML=scheduled.map(game=>{
        const ev=events.find(e=>eventMatchesScheduled(e,game));
        if(ev){
          ...
          const label=game.big_sky_game?'BIG SKY':'NON-CONFERENCE';
          return `<div class="fcs-game ${state} bigsky-row"><div class="fcs-time">${escapeHtml(d.time||game.time||'TBA')}</div><div class="fcs-matchup"><b>${escapeHtml(d.away)} @ ${escapeHtml(d.home)}</b><small>${scores} <span class="bigsky-game-tag">${label}</span></small></div><div class="fcs-score">${rightScore}</div><div class="fcs-tv">${escapeHtml(tv)}</div></div>`;
        }
        const label=game.big_sky_game?'BIG SKY':'NON-CONFERENCE';
        return `<div class="fcs-game scheduled bigsky-row"><div class="fcs-time">${escapeHtml(game.time||'TBA')}</div><div class="fcs-matchup"><b>${escapeHtml(game.displayAway)} @ ${escapeHtml(game.displayHome)}</b><small><span class="bigsky-game-tag">${label}</span></small></div><div class="fcs-score"><small>UPCOMING</small></div><div class="fcs-tv"></div></div>`;
      }).join('');
    }
  }
