...
"""
    ...
    def class_key(value):
        ...
        return {'fr':'FRESHMEN','freshman':'FRESHMEN','so':'SOPHOMORES','sophomore':'SOPHOMORES','jr':'JUNIORS','junior':'JUNIORS','sr':'SENIORS','senior':'SENIORS','5th':'5TH YEAR','5thyear':'5TH YEAR','gr':'GRADUATE','graduate':'GRADUATE'}.get(v,'OTHER')
    class_counts=Counter(class_key(p['year']) for p in players)
    counts={'OFFENSE':sum(unit(p['pos'])=='offense' for p in players),'DEFENSE':sum(unit(p['pos'])=='defense' for p in players),'SPECIALISTS':sum(unit(p['pos'])=='special' for p in players),'FRESHMEN':class_counts['FRESHMEN'],'SOPHOMORES':class_counts['SOPHOMORES'],'JUNIORS':class_counts['JUNIORS'],'SENIORS':class_counts['SENIORS'],'5TH YEAR':class_counts['5TH YEAR']}
  ...
  <div class="roster-snapshot-card"><span>{FIFTH_YEAR}</span><strong>5TH YEAR</strong><small>Graduate / extra-eligibility veterans</small></div>
    ...
    snapshot_start=text.find('<div class="roster-snapshot-grid">'); snapshot_end=text.find('<div class="roster-explorer"', snapshot_start)
    if snapshot_start < 0 or snapshot_end < 0: raise RuntimeError('Safety stop: roster snapshot markers missing')
    ...
    if 'data-roster-jump="roster-coaches"' not in text:
        needle='<button type="button" class="roster-section-nav-btn" data-roster-jump="roster-transfers">'; repl='<button type="button" class="roster-section-nav-btn" data-roster-jump="roster-coaches">🏈 <span>COACHES</span></button>\n  '+needle; text=text.replace(needle,repl,1)
...
def main():
    ...
    players=parse_roster(roster_print); validate_roster(players); photos=verified_photos(roster_page,players)
    print(f'Player profile links: {len(photos)} headshots resolved')
    if len(photos)<max(90,int(len(players)*0.90)): raise RuntimeError(f'Safety stop: only {len(photos)} verified player photos found for {len(players)} players')
    raw_coaches=parse_coaches(coaches_page); coaches=validate_coaches(raw_coaches); linkp=LinkParser(); linkp.feed(coaches_page); profile_links={}
    for href,label in linkp.links:
        if '/sports/football/roster/coaches/' not in href: continue
        label_key=norm(label); href_key=norm(href)
        for c in coaches:
            ck=norm(c['name']); parts=[norm(x) for x in c['name'].split()]
            if (ck and ck in label_key) or (parts and all(part in href_key for part in parts[-1:])): profile_links[c['name']]=absolute_url(href,COACHES_URL); break
    def resolve_coach(c):
        name=c['name']; profile=profile_links.get(name)
        if not profile: return name,''
        try:
            profile_page=fetch(profile); ip=ImageParser(); ip.feed(profile_page); key=norm(name)
            for im in ip.images:
                alt=norm(im['alt']); title=norm(im['title']); u=absolute_url(im['src'],profile)
                if valid_image_url(u) and key and (key==alt or key==title or key in alt or key in title): return name,u
            ...
            for im in ip.images:
                ...
                if valid_image_url(u): return name,u
        except Exception as exc: print(f'Coach photo warning: {name}: {exc}')
        return name,''
    coach_photos={}
